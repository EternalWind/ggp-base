...
public abstract class StateMachine {
	...
	private Stack<Long> profileStack = new Stack<Long>();
	private boolean isProfilingOn = true;

	protected void beginProfile() {
		if (isProfilingOn) {
			profileStack.push(System.currentTimeMillis());
		}
	}

	protected void endProfile(String profileName) {
		if (isProfilingOn) {
			long currentVal = 0;

			if (profiles.containsKey(profileName)) {
				currentVal = profiles.get(profileName);
			}

			long currentTime = System.currentTimeMillis();
			currentVal += currentTime - profileStack.pop();
			profiles.put(profileName, currentVal);
		}
	}

	public void showProfiles() {
		if (isProfilingOn) {
			for (Map.Entry<String, Long> e : profiles.entrySet()) {
	 ...
	 * <p>
	 * This method should only be called once, and it should be called before
	 * any other methods on the StateMachine.
	 */
	...
	/**
	 * Returns a list containing every move that is legal for the given role in
	 * the given state.
	 *
	 * @throws MoveDefinitionException
	 *             if the role has no legal moves. This indicates an error in
	 *             either the game description or the StateMachine
	 *             implementation.
	 */
	// TODO: There are philosophical reasons for this to return Set<Move> rather
	// than List<Move>.
	public abstract List<Move> getLegalMoves(MachineState state, Role role)
			throws MoveDefinitionException;

	/**
	 * Returns the next state of the game given the current state and a joint
	 * move list containing one move per role.
	 *
	 * @param moves
	 *            A list containing one move per role. The moves should be
	 *            listed in the same order as roles are listed by
	 *            {@link #getRoles()}.
	 * @throws TransitionDefinitionException
	 *             indicates an error in either the game description or the
	 *             StateMachine implementation.
	 */
	public abstract MachineState getNextState(MachineState state,
			List<Move> moves) throws TransitionDefinitionException;
	...
	// They are implemented for convenience, using the default ways of
	// generating
	// these objects, but they can be overridden to support machine-specific
	// objects.
	...
	/**
	 * Override this to perform some extra work (like trimming a cache) once per
	 * move.
	 * <p>
	...
	/**
	 * Override this to provide memory-saving destructive-next-state
	 * functionality.
	 * <p>
	...
	/**
	 * Override this to allow the state machine to be conditioned on a
	 * particular current state. This means that the state machine will only
	 * handle portions of the game tree at and below the given state; it no
	 * longer needs to properly handle earlier portions of the game tree. This
	 * constraint can be used to optimize certain state machine implementations.
	 * <p>
	 * CONTRACT: After calling this method, the state machine never deals with a
	 * state that is not "theState" or one of its descendants in the game tree.
	 */
	...
	/**
	 * Returns a list containing every joint move possible in the given state. A
	 * joint move consists of one move for each role, with the moves in the same
	 * ordering that their roles have in {@link #getRoles()}.
	 * <p>
	 * The list of possible joint moves is the Cartesian product of the lists of
	 * legal moves available for each player.
	 * <p>
	 * If only one player has more than one legal move, then the number of joint
	 * moves returned will equal the number of possible moves for that player.
	 */
	 ...
	 * the given role makes the given move. This will be a subset of the list of
	 * joint moves given by {@link #getLegalJointMoves(MachineState)}.
	 */
	public List<List<Move>> getLegalJointMoves(MachineState state, Role role,
	...
	/**
	 * Returns a list containing every possible next state of the game after the
	 * given state. The list will contain one entry for every possible joint
	 * move that could be played; as such, a single machine state could be
	...
	/**
	 * Returns a map from each move that is legal for the given role in the
	 * given state to the list of possible resulting states if that move is
	 * chosen.
	 * <p>
	 * If the given role is the only role with more than one legal move, then
	 * each list of states in the map will only contain one state.
	 */
	...
	protected void crossProductLegalMoves(List<List<Move>> legals,
			List<List<Move>> crossProduct, LinkedList<Move> partial) {
	...
	/**
	 * Returns a mapping from a role to the index of that role, as in the list
	 * returned by {@link #getRoles()}. This may be a faster way to check the
	 * index of a role than calling {@link List#indexOf(Object)} on that list.
	 */
	 ...
	 * are listed in the same order the roles are listed in the game rules,
	 * which is the same order in which they're returned by {@link #getRoles()}.
	 *
	 * @throws GoalDefinitionException
	 *             if there is no goal value or more than one goal value for any
	 *             one role in the given state. If this occurs when this is
	 *             called on a terminal state, this indicates an error in either
	 *             the game description or the StateMachine implementation.
	 */
	 ...
	 */
	public List<Move> getRandomJointMove(MachineState state, Role role,
			Move move) throws MoveDefinitionException {
		List<Move> random = new ArrayList<Move>();
		for (Role r : getRoles()) {
			if (r.equals(role)) {
	...
	/**
	 * Returns a random move from among the possible legal moves for the given
	 * role in the given state.
	 */
	 ...
	 * game.
	 * <p>
	 * The distribution among states is based on the possible joint moves. This
	 * is not necessarily uniform among the possible states themselves, as
	 * multiple joint moves may result in the same state.
	 */
	 ...
	 */
	public MachineState getRandomNextState(MachineState state, Role role,
			Move move) throws MoveDefinitionException,
			TransitionDefinitionException {
	...
	/**
	 * Returns a terminal state derived from repeatedly making random joint
	 * moves until reaching the end of the game.
	 *
	 * @param theDepth
	 *            an integer array, the 0th element of which will be set to the
	 *            number of state changes that were made to reach a terminal
	 *            state.
	 */
	public MachineState performDepthCharge(MachineState state,
			final int[] theDepth) throws TransitionDefinitionException,
	...
	public void getAverageDiscountedScoresFromRepeatedDepthCharges(
			final MachineState state, final double[] avgScores,
			final double[] avgDepth, final double discountFactor,
			final int repetitions) throws TransitionDefinitionException,
